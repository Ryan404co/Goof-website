<script lang="ts">
  ...
  const STEP = '4.2vw';
  $: lines = textList($page.data.texts, 'over.hero.headline_lines', [
    'streven om',
    'de wereld iets',
    'echts te geven'
  ]);
  $: paragraphs = [
    textString($page.data.texts, 'over.hero.paragraph_1', ''),
    textString($page.data.texts, 'over.hero.paragraph_2', ''),
    textString($page.data.texts, 'over.hero.paragraph_3', '')
  ].filter(Boolean);
</script>
